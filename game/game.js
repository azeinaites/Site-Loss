...
let frameCount = 0;
let charMood = 'sad'; // humor do personagem de fundo do palco (Teddy/AZT/Marcelo): 'sad' | 'happy'
...
// Tentativa inofensiva de incentivar o navegador a recolher a barra de endereço sozinho
// (efeito varia por navegador/versão; não altera nada se não houver como rolar).
function nudgeChromeCollapse() { try { window.scrollTo(0, 1); } catch (e) {} }
window.addEventListener('load', nudgeChromeCollapse);
window.addEventListener('orientationchange', () => setTimeout(nudgeChromeCollapse, 300));
...
// Personagens de fundo do palco (Teddy/AZT/Marcelo) — sprite sheet em vez de vídeo com
// canal alpha: o WebM com transparência não é suportado no motor do iOS (Safari e Chrome
// iOS, já que a Apple obriga os dois a usar o WebKit), então uma folha de PNG com
// transparência de verdade funciona igual em qualquer navegador, sem exceção.
const CHAR_ANIM_COLS = 8;      // 8 poses por humor
const CHAR_FRAME_W = 232, CHAR_FRAME_H = 416;
const CHAR_ANIM_SPEED = 9;     // frames de jogo por pose (~6-7 poses/seg a 60fps)
...
const teddyAnimSheet = loadImg('assets/teddy_anim.png');
...
const aztAnimSheet = loadImg('assets/azt_anim.png');
...
const marceloAnimSheet = loadImg('assets/marcelo_anim.png');
...
function triggerVictory() {
    ...
    stopMetronomeLoop();
    playVictorySfx();
    // o personagem de fundo (Teddy / Adriano / Marcelo) fica feliz quando o vilão é derrotado
    charMood = 'happy';
    updateTouchControlsVisibility();
}
...
// Personagem animado ao fundo do palco — Teddy na fase 1, "Adriano" (AZT) na fase 2,
// Marcelo na fase 3. Sprite sheet (não vídeo) pra funcionar com transparência real em
// qualquer navegador/plataforma: linha 0 = triste/bravo, linha 1 = feliz (pós-vitória).
function drawPhaseChar() {
    const phase = PHASES[currentPhaseIndex];
    const sheet = phase.charSheet;
    ...
    const row = charMood === 'happy' ? 1 : 0;
    const frameX = Math.floor(frameCount / CHAR_ANIM_SPEED) % CHAR_ANIM_COLS;
    ctx.drawImage(sheet, frameX * CHAR_FRAME_W, row * CHAR_FRAME_H, CHAR_FRAME_W, CHAR_FRAME_H,
        rx, cw.y - h, w, h);
}
